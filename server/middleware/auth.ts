...
export default defineEventHandler(async (event) => {
  ...
  if (["JWT_SECRET", "G_CLIENT_ID", "G_CLIENT_SECRET"].find(k => !process.env[k])) {
    ...
    if (["/api/s", "/api/proxy", "/api/latest", "/api/mcp", "/api/tts", "/api/llm", "/api/enable-login", "/api/carousel", "/api/playlist", "/api/summary", "/api/carousel-tts", "/api/carousel-summary-tts"].every(p => !url.pathname.startsWith(p)))
